...
const ThemeManager = {
  ...
  setTheme(themeName, isInit = false) {
    ...
    // 同步首页背景动效配色
    if (typeof HomeBackground !== 'undefined') HomeBackground.setTheme(themeName);
  },
...
const StorageManager = {
  ...
  _dirty: false,
  _maxSaveTimer: null,
  _hoveringDraggable: false,
  _lastPointerMoveTime: 0,
  _isBackground: false,

  /* 初始化：检测 API 支持并恢复数据 */
  async init() {
    this._bindSaveSensors();
    ...
    if (window.electronAPI) {
      // 优先从 localStorage 恢复上次工作目录（绕过主进程 EPERM 写盘限制）
      const savedWorkDir = localStorage.getItem('gm-assistant-workdir');
      if (savedWorkDir) {
        const dirExists = await window.electronAPI.checkDir(savedWorkDir);
        if (dirExists) {
          AppState.workDirPath = savedWorkDir;
      ...
      // 只有 localStorage 无记录时才走 getLastDir
      if (!AppState.workDirPath) {
        const lastDir = await window.electronAPI.getLastDir();
        if (lastDir) {
          const dirExists = await window.electronAPI.checkDir(lastDir);
          if (dirExists) {
    ...
    // 退出分屏后：延迟 1 秒关闭外部加载遮罩，防止遮罩与页面内容同时消失
    if (urlModuleId && window.electronAPI) {
      setTimeout(() => {
  ...
  async pickWorkDir() {
    try {
      ...
      if (window.electronAPI) {
        ...
        AppState.useFileSystemAPI = true;
        localStorage.setItem('gm-assistant-workdir', dirPath);  // 持久化（绕过主进程 EPERM）
        this.hideWorkdirPrompt();
        await this.loadModulesFromFileSystem();
        UIRender.renderModuleList();
        return;
  ...
  async loadModulesFromFileSystem() {
    ...
    if (window.electronAPI && AppState.workDirPath) {
      try {
          ...
          try {
            ...
            if (text) {
              ...
              if (data && data.id && data.name) {
                ...
                // 保留磁盘上的实际文件名，避免同名模组保存时互相覆盖
                data.fileName = fileName;
                // 修复复制文件导致的重复 id（副本会共享同一个 id）
                if (seenIds.has(data.id)) {
                  data.id = AppState.generateUUID();
                  needResave.push(data);
                }
                seenIds.add(data.id);
        ...
        // 重复 id 已重新生成，立即写回磁盘，避免下次加载再次重复
        for (const mod of needResave) {
          await this.saveModuleToFileSystem(mod);
        }
        // 方案四：检测到损坏存档时提示用户（只提示，不自动恢复）
        if (corrupted.length && window.electronAPI.showCorruptModuleDialog) {
          await window.electronAPI.showCorruptModuleDialog(corrupted, AppState.workDirPath);
        }
      } catch (e) {
  ...
  async saveModuleToFileSystem(module) {
    ...
    if (window.electronAPI && AppState.workDirPath) {
      try {
        ...
        const ok = await window.electronAPI.writeModuleFile(filePath, module);
        if (!ok) {
  ...
  /* 生成模组文件名（优先使用加载时保留的磁盘文件名，避免同名模组互相覆盖） */
  ...
  /* 触发保存（8秒防抖：操作停顿8秒后才保存；后台降频30秒） */
  scheduleSave() {
    if (this.saveTimer) clearTimeout(this.saveTimer);
    UIRender.showSaveStatus('saving');
    this._dirty = true;
    this._ensureMaxSaveGuard();
    const delay = this._isBackground ? 30000 : 2500;
    this.saveTimer = setTimeout(() => {
      this._trySave();
    }, delay);
  },

  /* 保存前检查：鼠标悬停在可拖拽元素上且最近5秒内动过，判定可能拖拽中，推迟保存 */
  _trySave() {
    const now = Date.now();
    if (this._hoveringDraggable && (now - this._lastPointerMoveTime) < 5000) {
      this.saveTimer = setTimeout(() => this._trySave(), 1000);
      return;
    }
    this.saveNow();
  },

  /* 绑定保存时机传感事件：hover 预警 + 后台切换 */
  _bindSaveSensors() {
    document.addEventListener('pointermove', (e) => {
      this._lastPointerMoveTime = Date.now();
      this._hoveringDraggable = !!(e.target && e.target.closest && e.target.closest('[data-note-id]'));
    });
  ...
  /* 兜底：长时间连续操作时，距上次成功保存超过5分钟强制保存一次 */
  _ensureMaxSaveGuard() {
    if (this._maxSaveTimer) return;
    this._maxSaveTimer = setInterval(() => {
      if (this._dirty && Date.now() - this._lastSaveTime > 300000) {
        console.log('[saveNow] 超过5分钟未保存，触发兜底保存');
        this.saveNow();
      }
    }, 30000);
  },
...
const UIRender = {
  ...
  renderModuleList() {
    ...
    AppState.modules.forEach((mod, index) => {
      ...
      card.addEventListener('click', (e) => {
        ...
        App.openModule(mod.id, { transition: true });
      });
  ...
  renderModuleListPage() {
    ...
    AppState.modules.forEach((mod, index) => {
      ...
      panel.className = 'gallery-panel' + (index === 0 ? ' expanded' : '');
      panel.setAttribute('data-module-id', mod.id);

      const systemInfo = SystemManager.getSystemInfo(mod.system);
      const statusLabel = AppState.getStatusLabel(mod.status);
      const coverUrl = this._coverImageToUrl(mod.coverImage);
      const bgStyle = coverUrl
        ? ' style="background-image:url(\'' + coverUrl + '\');"'
        : '';

      panel.innerHTML =
        '<div class="gallery-panel-bg"' + bgStyle + '></div>' +
        '<div class="gallery-panel-scrim"></div>' +
        '<div class="gallery-panel-info-collapsed">' +
          '<div class="gallery-panel-name">' + this.escapeHtml(mod.name) + '</div>' +
        '</div>' +
        '<div class="gallery-panel-info-expanded">' +
          '<div class="gallery-panel-name">' + this.escapeHtml(mod.name) + '</div>' +
          '<div class="gallery-panel-meta">' + this.escapeHtml(systemInfo.name) + ' · ' + statusLabel + '</div>' +
        '</div>' +
        '<button class="gallery-cover-btn" title="更换封面" data-id="' + mod.id + '">' +
          '<span class="icon"><svg><use href="#i-image"/></svg></span>' +
        '</button>' +
        '<button class="gallery-delete-btn" title="删除模组" data-id="' + mod.id + '">' +
          '<span class="icon"><svg><use href="#i-trash"/></svg></span>' +
        '</button>';

      panel.addEventListener('click', (e) => {
        if (e.target.closest('.gallery-cover-btn') || e.target.closest('.gallery-delete-btn')) return;
        App.openModule(mod.id, { transition: true });
      });

      panel.addEventListener('mouseenter', () => {
        this._expandGalleryPanel(panel);
      });

      const coverBtn = panel.querySelector('.gallery-cover-btn');
      coverBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        this.pickModuleCover(mod.id);
      });

      const deleteBtn = panel.querySelector('.gallery-delete-btn');
      deleteBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        App.showDeleteConfirm(mod);
      });
    ...
    this._ensureGalleryScroll(container);
  },

  /* 展开指定画廊面板（手风琴） */
  _expandGalleryPanel(panel) {
    if (this._galleryDragging) return;
    const container = document.getElementById('modulesFlowingMenu');
    if (!container) return;
    container.querySelectorAll('.gallery-panel.expanded').forEach(p => {
  ...
  /* 绑定画廊横向滚动（滚轮 + 拖动） */
  _ensureGalleryScroll(container) {
    if (this._galleryScrollBound) return;
    this._galleryScrollBound = true;
    ...
    window.addEventListener('mouseup', () => {
      if (!isDown) return;
      isDown = false;
      this._galleryDragging = false;
      container.classList.remove('dragging');
      this._galleryDragMoved = moved;
      setTimeout(() => { this._galleryDragMoved = false; }, 0);
    });

    // 捕获阶段：拖动后阻止面板 click 打开模组
    container.addEventListener('click', (e) => {
      if (this._galleryDragMoved) {
        e.stopPropagation();
        e.preventDefault();
        this._galleryDragMoved = false;
      }
    }, true);
  },

  /* 把 coverImage 转成可加载的 URL（兼容旧 data URL 与新的 covers/ 相对路径） */
  _coverImageToUrl(coverImage) {
    if (!coverImage) return '';
    // 旧数据：base64 data URL 直接使用
    if (coverImage.startsWith('data:')) return coverImage;
    // 新数据：相对 covers/ 路径 → file:// 绝对路径
    if (AppState.workDirPath) {
      const abs = AppState.workDirPath.replace(/\\/g, '/') + '/' + coverImage;
  ...
  /* 选择并设置模组封面 */
  async pickModuleCover(moduleId) {
    const mod = AppState.findModule(moduleId);
    if (!mod) return;

    // Electron 分支：把原图复制到工作目录 covers/ 下，JSON 只存相对标识，保持原图清晰度
    if (window.electronAPI && AppState.workDirPath) {
      try {
        const srcPath = await window.electronAPI.pickImageFile();
        if (!srcPath) return;

        const coversDir = AppState.workDirPath + '/covers';
        await window.electronAPI.ensureDir(coversDir);

        // 保留原始扩展名，确保 file:// 能正确识别图片类型
        const dotIdx = srcPath.lastIndexOf('.');
        const slashIdx = Math.max(srcPath.lastIndexOf('/'), srcPath.lastIndexOf('\\'));
        const ext = dotIdx > slashIdx ? srcPath.slice(dotIdx).toLowerCase() : '';
        // 每次生成唯一文件名，避免覆盖正在被渲染进程使用的同名文件
        // （覆盖同名文件会遇到 Windows 文件锁 / 浏览器缓存，导致封面不更新）
        const fileName = moduleId + '_' + Date.now() + ext;
        const relPath = 'covers/' + fileName;

        // 清理旧封面文件，避免垃圾堆积（旧文件可能正被占用，删除失败可忽略）
        if (mod.coverImage && mod.coverImage.startsWith('covers/')) {
          try {
            await window.electronAPI.deleteFile(AppState.workDirPath + '/' + mod.coverImage);
          } catch (e) {
            console.warn('[pickModuleCover] 清理旧封面失败(可忽略)', e);
          }
        }

        const ok = await window.electronAPI.copyFile(srcPath, coversDir + '/' + fileName);
        if (!ok) { alert('封面设置失败，请重试'); return; }

        console.log('[pickModuleCover] 封面文件已复制，coverImage 更新为:', relPath);
        mod.coverImage = relPath;
        const panel = document.querySelector('.gallery-panel[data-module-id="' + moduleId + '"]');
        if (panel) {
          const bg = panel.querySelector('.gallery-panel-bg');
          if (bg) bg.style.backgroundImage = 'url(\'' + this._coverImageToUrl(mod.coverImage) + '\')';
        }
        StorageManager.scheduleSave();
      } catch (err) {
        console.error('[pickModuleCover] 封面设置失败', err);
        alert('封面设置失败，请重试');
      }
      return;
    }

    // 浏览器分支：降级为压缩 data URL（保持原有行为）
    ...
    input.onchange = async (e) => {
      ...
      if (!file) return;
      const mod2 = AppState.findModule(moduleId);
      if (!mod2) return;
      try {
        const dataUrl = await this._compressCoverImage(file);
        mod2.coverImage = dataUrl;
        const panel = document.querySelector('.gallery-panel[data-module-id="' + moduleId + '"]');
        if (panel) {
          const bg = panel.querySelector('.gallery-panel-bg');
  ...
  /* 压缩封面图片为 JPEG 缩略图（最长边 800px） */
  _compressCoverImage(file) {
    return new Promise((resolve, reject) => {
      const reader = new FileReader();
      reader.onload = () => {
        const img = new Image();
  ...
  switchPage(page) {
    // 切换页面时退出沉浸模式（回到常驻栏）
    if (typeof ChromeManager !== 'undefined') ChromeManager._exit();
    ...
    // 首页/模组列表页共享波浪背景：跟随当前页面切换挂载点
    if (typeof HomeBackground !== 'undefined') HomeBackground.setPage(page);
  ...
  /* 页面过场：渐入纯色遮罩 → 执行切换 → 渐出 */
  runPageTransition(switchAction, opts = {}) {
    const overlay = document.getElementById('pageTransitionOverlay');
    if (!overlay) { switchAction(); return Promise.resolve(); }

    const isDark = ThemeManager.currentTheme === 'dark-forest';
    overlay.style.background = isDark ? '#000000' : '#f8f6f2';

    const fadeIn = (opts.fadeIn != null) ? opts.fadeIn : 250;
    const fadeOut = (opts.fadeOut != null) ? opts.fadeOut : 400;
...
const ShortcutManager = {
  ...
  _defaults: {
    ...
    zoomStep: 0.02  // 滚轮缩放步长（带团板流程单元画布）
  },
  ...
  async init() {
    ...
    this._bindings = { ...this._defaults.shortcuts, ...(cfg.shortcuts || {}) };
    this._zoomStep = cfg.zoomStep ?? this._defaults.zoomStep;
  ...
  async _saveConfig() {
    try {
      if (window.electronAPI && window.electronAPI.saveConfig) {
        await window.electronAPI.saveConfig({ shortcuts: this._bindings, zoomStep: this._zoomStep });
      }
  ...
  _renderDialog() {
    ...
    // 缩放设置
    html += '<div class="shortcut-section-title">缩放</div>';
    html += '<div class="shortcut-item shortcut-zoom-item">';
    html += '<span class="shortcut-item-label">滚轮步长</span>';
    html += '<div class="shortcut-zoom-control">';
    html += '<span class="shortcut-zoom-label">精细</span>';
    html += `<input type="range" class="shortcut-zoom-slider" min="1" max="10" value="${Math.round(this._zoomStep * 100)}" oninput="ShortcutManager._onZoomStepChange(this.value)">`;
    html += '<span class="shortcut-zoom-label">快速</span>';
    html += `<span class="shortcut-zoom-value">${Math.round(this._zoomStep * 100)}%</span>`;
    html += '</div>';
    html += '</div>';
  ...
  /* 滚轮缩放步长变更 */
  _onZoomStepChange(value) {
    this._zoomStep = parseInt(value) / 100;
    // 更新百分比显示
    const display = document.querySelector('.shortcut-zoom-value');
  ...
  /* 恢复默认 */
  resetDefaults() {
    this._bindings = { ...this._defaults.shortcuts };
    this._zoomStep = this._defaults.zoomStep;
    this._recordingAction = null;
    this._renderDialog();
    this._saveConfig();
  }
...
/* ==========================================================================
 * 沉浸模式模块
 * 隐藏顶部栏（专注编辑 / 全屏带团板）。通过把顶部栏搬入固定浮层 + transform
 * 位移实现，内容区始终占满全屏、不回流、不跳动。
 * ========================================================================== */
const ChromeManager = {
  _mode: null,        // null | 'editor' | 'board'
  _chromeEl: null,    // #immersiveChrome 固定浮层容器
  _hotzoneEl: null,   // #immersiveHotzone 顶部热区（编辑器 hover 滑出）
  _exitBtnEl: null,   // #immersiveExitBtn 悬浮退出按钮
  _origin: [],        // 被搬移元素的原位置记录（退出时还原）
  _hideTimer: null,   // 编辑器自动隐藏延时

  /* 初始化：动态创建浮层/热区/退出按钮，并绑定事件 */
  init() {
    if (document.getElementById('immersiveChrome')) return;

    const chrome = document.createElement('div');
    chrome.id = 'immersiveChrome';
    document.body.appendChild(chrome);
    this._chromeEl = chrome;

    const hotzone = document.createElement('div');
    hotzone.id = 'immersiveHotzone';
    document.body.appendChild(hotzone);
    this._hotzoneEl = hotzone;

    const exitBtn = document.createElement('button');
    exitBtn.id = 'immersiveExitBtn';
    exitBtn.title = '退出专注/全屏 (Esc)';
    exitBtn.innerHTML = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 3 3 3 3 9"/><polyline points="15 21 21 21 21 15"/><line x1="3" y1="3" x2="10" y2="10"/><line x1="21" y1="21" x2="14" y2="14"/></svg>';
    exitBtn.addEventListener('click', () => this._exit());
    document.body.appendChild(exitBtn);
    this._exitBtnEl = exitBtn;

    hotzone.addEventListener('mouseenter', () => this._reveal());
    chrome.addEventListener('mouseenter', () => this._cancelHide());
    chrome.addEventListener('mouseleave', () => this._scheduleHide());
    hotzone.addEventListener('mouseleave', () => this._scheduleHide());

    document.addEventListener('keydown', (e) => this._onKeydown(e), true);
  },

  /* 是否处于沉浸模式 */
  isActive() { return !!this._mode; },

  /* 快捷键：Esc 退出，Ctrl+Shift+F 切换 */
  _onKeydown(e) {
    if (e.key === 'Escape' && this._mode) {
      e.preventDefault();
      e.stopPropagation();
      this._exit();
      return;
  ...
  /* 入口：按当前页面切换（编辑器专注 / 带团板全屏） */
  toggle() {
    if (this._mode) { this._exit(); return; }
    const page = (typeof AppState !== 'undefined') ? AppState.currentPage : null;
    if (page === 'editor') this._enter('editor');
    else if (page === 'board') this._enter('board');
  },

  /* 进入沉浸模式 */
  _enter(mode) {
    if (!this._chromeEl) this.init();
    this._exit();

    const els = this._collect(mode);
    if (!els.length) return;
    this._mode = mode;

    this._origin = els.map(el => ({ el, parent: el.parentNode, next: el.nextSibling }));
    els.forEach(el => this._chromeEl.appendChild(el));

    document.body.classList.add('immersive');
    this._chromeEl.style.display = 'flex';
    this._chromeEl.classList.remove('open');
    this._sync();

    if (typeof BoardManager !== 'undefined' && BoardManager._updateToolsPosition) {
      BoardManager._updateToolsPosition();
    }
  },

  /* 收集要隐藏的顶部栏（按顺序），navbar 仅在可见时纳入 */
  _collect(mode) {
    const list = [];
    const navbar = document.getElementById('navbar');
    if (navbar && getComputedStyle(navbar).display !== 'none') list.push(navbar);
    if (mode === 'editor') {
      list.push(document.querySelector('#pageEditor .editor-toolbar'));
      list.push(document.getElementById('findReplaceBar'));
    } else {
      list.push(document.querySelector('#pageBoard .board-toolbar'));
      list.push(document.getElementById('boardSearchBar'));
      list.push(document.getElementById('boardUnitHeader'));
    }
  ...
  _exit() {
    this._cancelHide();
    // 倒序还原：同一父节点下连续的栏，先还原靠后的 next 兄弟，避免 insertBefore 失败回退到末尾
    for (let i = this._origin.length - 1; i >= 0; i--) {
      const { el, parent, next } = this._origin[i];
      if (!parent) continue;
      try {
    ...
    if (this._chromeEl) {
      this._chromeEl.classList.remove('open');
      this._chromeEl.style.display = 'none';
    }
    this._sync();
    if (typeof BoardManager !== 'undefined' && BoardManager._updateToolsPosition) {
      BoardManager._updateToolsPosition();
  ...
  _reveal() {
    ...
    this._cancelHide();
    if (this._chromeEl) this._chromeEl.classList.add('open');
  },
  ...
  /* 移出后延时隐藏（仅编辑器；带团板由退出按钮控制，不自动隐藏） */
  _scheduleHide() {
    if (this._mode !== 'editor') return;
    this._cancelHide();
    this._hideTimer = setTimeout(() => {
      this._hideTimer = null;
      if (this._chromeEl) this._chromeEl.classList.remove('open');
    }, 300);
  },

  /* 同步退出按钮 / 热区的可见性 */
  _sync() {
    if (this._exitBtnEl) this._exitBtnEl.style.display = this._mode ? 'flex' : 'none';
    if (this._hotzoneEl) this._hotzoneEl.style.display = (this._mode === 'editor') ? 'block' : 'none';
  }
};
...
const App = {
  ...
  async init() {
    ...
    try {
      if (typeof SharedBridge !== 'undefined' && SharedBridge.isInIframe()) {
        ...
        // 分屏模式：文档编辑器入库后自动在画布创建便签 + 同步数据库面板
        SharedBridge.on('ENTRY_ADDED', (data) => {
          if (!data.dbKey || !data.entryId) return;
          ...
          if (data.entry) {
            ...
            if (mod && mod.databases) {
              for (const groupKey of Object.keys(mod.databases)) {
                const groupDb = mod.databases[groupKey];
                if (groupDb && groupDb[data.dbKey] && Array.isArray(groupDb[data.dbKey])) {
                  const existing = groupDb[data.dbKey].find(e => e.id === data.entryId);
                  if (!existing) {
                    groupDb[data.dbKey].push(data.entry);
                  }
                }
              }
            }
          ...
          if (typeof DatabaseManager !== 'undefined') {
            ...
            if (DatabaseManager._currentDbKey === data.dbKey) {
              DatabaseManager.renderEntries(data.dbKey);
            }
          }
          if (typeof BoardManager !== 'undefined') {
            BoardManager.renderDbList();
            if (BoardManager._currentDbKey === data.dbKey) {
              BoardManager.renderDbEntries(data.dbKey);
            }
        ...
        // 分屏模式：数据库删除同步
        SharedBridge.on('ENTRY_DELETED', (data) => {
          if (!data.dbKey || !data.entryId) return;
          ...
          if (mod && mod.databases) {
            for (const groupKey of Object.keys(mod.databases)) {
              const groupDb = mod.databases[groupKey];
              if (groupDb && groupDb[data.dbKey] && Array.isArray(groupDb[data.dbKey])) {
                const idx = groupDb[data.dbKey].findIndex(e => e.id === data.entryId);
                if (idx >= 0) {
                  groupDb[data.dbKey].splice(idx, 1);
                }
              }
            }
          }
        ...
        // 分屏模式：自定义类型新增同步（两屏副本保持一致，避免退出分屏整字段覆盖时丢失）
        SharedBridge.on('DB_TYPE_ADDED', (data) => {
          if (!data.dbKey || !data.typeConfig) return;
          const mod = AppState.currentModule;
          if (!mod) return;
          if (!mod.customDbTypes) mod.customDbTypes = {};
          mod.customDbTypes[data.dbKey] = data.typeConfig;
          const allGroups = mod.databases || {};
          for (const groupKey of Object.keys(allGroups)) {
            if (!allGroups[groupKey][data.dbKey]) {
              allGroups[groupKey][data.dbKey] = [];
            }
          }
          if (typeof DatabaseManager !== 'undefined') DatabaseManager.renderDbList();
          if (typeof BoardManager !== 'undefined' && BoardManager._dbPanelOpen) BoardManager.renderDbList();
        });

        // 分屏模式：数据库类型删除同步
        SharedBridge.on('DB_TYPE_DELETED', (data) => {
          if (!data.dbKey) return;
          const mod = AppState.currentModule;
          if (!mod) return;
          if (data.isBuiltin) {
            if (!Array.isArray(mod.hiddenDbTypes)) mod.hiddenDbTypes = [];
            if (!mod.hiddenDbTypes.includes(data.dbKey)) mod.hiddenDbTypes.push(data.dbKey);
          } else if (mod.customDbTypes) {
            delete mod.customDbTypes[data.dbKey];
          }
          const allGroups = mod.databases || {};
          for (const groupKey of Object.keys(allGroups)) {
            if (allGroups[groupKey][data.dbKey]) delete allGroups[groupKey][data.dbKey];
          }
          const fallback = Object.keys(DatabaseManager.getMergedDbConfig()).find(k => k === 'characters') || Object.keys(DatabaseManager.getMergedDbConfig())[0] || 'characters';
          if (typeof DatabaseManager !== 'undefined') {
            if (DatabaseManager._currentDbKey === data.dbKey) DatabaseManager._currentDbKey = null;
            DatabaseManager.renderDbList();
            DatabaseManager.selectDb(fallback);
          }
          if (typeof BoardManager !== 'undefined' && BoardManager._dbPanelOpen) {
            BoardManager.renderDbList();
  ...
  goHome() {
    ...
    // 已在首页时无需过场，直接刷新；否则播放淡入淡出过场
    if (AppState.currentPage === 'home') {
      doGoHome();
    } else {
      UIRender.runPageTransition(doGoHome);
    }
  },
  ...
  confirmCreate() {
    ...
    // 进入编辑器页面
    this.openModule(module.id, { transition: true });
  ...
  handleRunSession() {
    ...
    UIRender.runPageTransition(doOpenModules);
  },
  ...
  openModule(moduleId, opts) {
    ...
    if (opts && opts.transition) {
      UIRender.runPageTransition(doOpen);
    } else {
      doOpen();
    }
  },
  ...
  async enterSplitScreen() {
    ...
    // 显示外部加载遮罩（主进程创建独立窗口，覆盖在主窗口之上）
    if (window.electronAPI) {
      await window.electronAPI.showLoading(ThemeManager.currentTheme || 'default');
      // 短暂延迟确保遮罩窗口渲染完成
  ...
  switchToBoard() {
    ...
    // 从编辑器进入带团板：同款淡入淡出过场
    UIRender.runPageTransition(doSwitch);
  },
  ...
  switchToEditor() {
    ...
    // 从带团板切回编辑器：过场稍快
    UIRender.runPageTransition(doSwitch, { fadeIn: 150, fadeOut: 250 });
  },
  ...
  showDeleteConfirm(mod) {
    this.showConfirm(
      ...
      async () => {
        ...
        if (itemEl) {
          itemEl.classList.add('gallery-panel-removing');
...
const DocEditor = {
  ...
  init() {
    ...
    const doInit = () => {
      ...
      tinymce.init({
        ...
        resize: false,
        content_style: `
          body { font-family: 'Microsoft YaHei', 'PingFang SC', sans-serif; font-size: 16px; line-height: 1.8; padding: 20px; max-width: 100%; margin: 0 auto; transition: max-width 0.4s cubic-bezier(0.25, 0.8, 0.25, 1); }
        ...
        setup(editor) {
          ...
          editor.on('init', () => {
            ...
            /* 应用阅读视图状态（若在编辑器就绪前已切换过） */
            self._applyReadingView();
            /* 编辑器 iframe 内点击关闭右键菜单（用 document 而非 body，确保点击正文下方的空白区域也能关闭） */
            const editorDoc = editor.getDoc();
            if (editorDoc) {
              editorDoc.addEventListener('click', (e) => {
                const menu = document.getElementById('editorContextMenu');
                if (menu && menu.classList.contains('visible') && !e.target.closest('.editor-context-menu')) {
                  self.hideContextMenu();
                }
              });
              /* iframe 内点击也关闭字号/主题下拉（跨文档事件不冒泡到主 document） */
              editorDoc.addEventListener('mousedown', () => {
                const fsMenu = document.getElementById('fontSizeDropdown');
                if (fsMenu) fsMenu.classList.remove('show');
                const thMenu = document.getElementById('themeDropdown');
    ...
    if (imgInput) {
      imgInput.addEventListener('change', (e) => {
        ...
        reader.onload = (ev) => {
          if (this._editor) {
            this._editor.focus();
            this._editor.insertContent('<img src="' + ev.target.result + '" />');
          }
        };
        reader.readAsDataURL(file);
        imgInput.value = '';
    ...
    /* 点击外部关闭字号预设下拉（mousedown，因按钮 click 已 stopPropagation） */
    document.addEventListener('mousedown', (e) => {
      const menu = document.getElementById('fontSizeDropdown');
      const wrap = document.getElementById('fontSizeWrap');
  ...
  setHeading(level) {
    ...
    this._editor.focus();
    this._editor.execCommand('FormatBlock', false, String(level).toLowerCase());
  },
  ...
  toggleFontSizeMenu(e) {
    ...
    if (willShow) {
      /* 用 fixed 定位脱离工具栏 overflow 裁剪，动态对齐到字号输入框正下方 */
      const wrap = document.getElementById('fontSizeWrap');
      if (wrap) {
  ...
  async clearFontColor() {
    ...
    this.updateToolbarState();
    const indicator = document.getElementById('fontColorIndicator');
    if (indicator) indicator.style.backgroundColor = '#333333';
    /* 关闭面板 */
    const picker = document.getElementById('docColorPicker');
  ...
  updateToolbarState() {
    ...
    const colorIndicator = document.getElementById('fontColorIndicator');
    if (colorBtn && colorIndicator) {
      ...
      if (selColor) {
        ...
        colorIndicator.style.backgroundColor = '#333333';
  ...
  setZoom(value) {
    ...
    // TinyMCE 用 iframe 渲染正文，原 div 被隐藏，需同步缩放 iframe 内 body 才能生效
    if (this._editor && this._editor.getBody()) {
      this._editor.getBody().style.zoom = scale;
    }
    const label = document.getElementById('editorZoomLabel');
    if (label) label.textContent = value + '%';
    // 同步滑动条填充进度（min=50 max=200）
    const slider = document.getElementById('editorZoomSlider');
    if (slider) {
      const pct = ((value - 50) / 150) * 100;
      slider.style.setProperty('--val', pct + '%');
    }
  },

  /* 应用阅读视图状态到 TinyMCE 正文 body（正文居中限宽，两侧留白） */
  _applyReadingView() {
    const body = this._editor && this._editor.getBody();
    if (!body) return;
    body.style.maxWidth = this._readingView ? '820px' : '100%';
    const btn = document.getElementById('btnReadingView');
    if (btn) btn.classList.toggle('active', !!this._readingView);
  },

  /* 切换阅读视图（Word 风格：正文居中、两侧留白，适合阅读） */
  toggleReadingView() {
    this._readingView = !this._readingView;
    this._applyReadingView();
  },
  ...
  _initColorPicker() {
    const colorBtn = document.getElementById('fontColorBtn');
    const picker = document.getElementById('docColorPicker');
    if (!colorBtn || !picker) return;
    ...
    colorBtn.addEventListener('mousedown', (e) => {
      ...
      if (willShow) {
        /* 用 fixed 定位脱离工具栏 overflow 裁剪，动态对齐到按钮正下方 */
    ...
    /* 系统取色器 */
    const sysColorPicker = document.getElementById('docSysColorPicker');
    if (sysColorPicker) {
      sysColorPicker.addEventListener('mousedown', (e) => {
        e.stopPropagation();
        this._sysColorPickerOpen = true;
      });
      sysColorPicker.addEventListener('input', async (e) => {
        e.stopPropagation();
        /* 用 formatter.apply 直接操作 DOM，不触发焦点切换，避免取色器窗口被关闭 */
        await this.ready();
        if (this._editor) {
          this._editor.formatter.apply('forecolor', { value: e.target.value });
          this._markDirty();
          const indicator = document.getElementById('fontColorIndicator');
          if (indicator) indicator.style.backgroundColor = e.target.value;
        }
      });
      sysColorPicker.addEventListener('change', async (e) => {
        e.stopPropagation();
        this._sysColorPickerOpen = false;
        /* 选完之后焦点还给编辑器 */
        await this.ready();
    ...
    /* 清除颜色按钮 */
    const clearBtn = picker.querySelector('.color-clear');
    if (clearBtn) {
    ...
    /* 点击外部关闭（系统取色器打开时跳过，避免取色器弹窗关闭面板） */
    document.addEventListener('mousedown', (e) => {
      if (this._sysColorPickerOpen) return;
      if (picker.style.display !== 'none' && !picker.contains(e.target) && e.target !== colorBtn && !colorBtn.contains(e.target)) {
        picker.style.display = 'none';
      }
    });
  },

  /* 工具栏横向滚轮：鼠标在工具栏上滚轮时横向滚动，不冒泡到文档区域 */
  _initToolbarScroll() {
  ...
  showContextMenu(e) {
    ...
    requestAnimationFrame(() => {
      ...
      // TinyMCE 的 contextmenu 事件 clientX/Y 是 iframe 内部视口坐标，
      // 需加上 iframe 元素在主文档中的偏移，转换为 fixed 定位所用的主文档坐标
      let iframeLeft = 0, iframeTop = 0;
      if (this._editor && this._editor.getWin()) {
        const iframeEl = this._editor.getWin().frameElement;
        if (iframeEl) {
      ...
      let left = iframeLeft + e.clientX;
      let top = iframeTop + e.clientY;
  ...
  renderTocTree() {
    ...
    /* 回退：从编辑器 iframe 正文中提取标题 */
    const body = this._editor && this._editor.getBody ? this._editor.getBody() : null;
  ...
  scrollToHeadingById(idOrTitle) {
    const doc = this._editor && this._editor.getDoc ? this._editor.getDoc() : null;
    const body = this._editor && this._editor.getBody ? this._editor.getBody() : null;
    if (!doc || !body) return;

    /* 先尝试按 id 查找 */
    let target = doc.getElementById(idOrTitle);
  ...
  updateTocHighlight() {
    ...
    headings.forEach((h) => {
      if (h.offsetTop <= scrollTop + 60) {
        activeId = h.id;
        activeTitle = h.textContent.trim().substring(0, 50);
      }
    });
    ...
    document.querySelectorAll('.toc-item').forEach(item => {
      ...
      const matchByTitle = activeTitle && item.getAttribute('data-title') === activeTitle;
...
const DatabaseManager = {
    ...
    App.showConfirm(
      ...
      () => {
        ...
        if (idx >= 0) {
          ...
          try {
            if (typeof SharedBridge !== 'undefined' && SharedBridge.isInIframe()) {
              SharedBridge.send('ENTRY_DELETED', { dbKey, entryId });
            }
          } catch(e) { /* ignore */ }
        }
      }
  ...
  addCustomType() {
    ...
    // 立即保存并同步另一屏（与 addToDatabase 一致，避免防抖未触发就退出分屏导致类型丢失）
    (async () => {
      await StorageManager.saveNow();
      try {
        if (typeof SharedBridge !== 'undefined' && SharedBridge.isInIframe()) {
          SharedBridge.send('DB_TYPE_ADDED', {
            dbKey: key,
            typeConfig: mod.customDbTypes[key]
          });
        }
      } catch (e) { /* ignore */ }
    })();
  ...
  deleteCustomType(dbKey) {
    ...
    // 立即保存并同步另一屏（删除类型同样避免分屏下两屏副本不一致导致覆盖）
    (async () => {
      await StorageManager.saveNow();
      try {
        if (typeof SharedBridge !== 'undefined' && SharedBridge.isInIframe()) {
          SharedBridge.send('DB_TYPE_DELETED', {
            dbKey: dbKey,
            isBuiltin: isBuiltin
          });
        }
...
/* ==========================================================================
 * 性能监控模块 (PerfMonitor) — 仅开发版
 * Ctrl+Alt+P 切换显示，打包版不加载
 * ========================================================================== */
const PerfMonitor = {
  _enabled: false,
  _el: null,
  ...
  _setup() {
    ...
    this._el.id = 'perf-monitor';
    Object.assign(this._el.style, {
      position: 'fixed', top: '10px', right: '10px', zIndex: '99999',
      background: 'rgba(0,0,0,0.78)', color: '#ccc', padding: '6px 10px',
      borderRadius: '4px', fontSize: '12px', fontFamily: 'Consolas, monospace',
      pointerEvents: 'none', display: 'none',
      backdropFilter: 'blur(6px)', WebkitBackdropFilter: 'blur(6px)',
      border: '1px solid rgba(255,255,255,0.1)', minWidth: '160px'
    });
    this._el.innerHTML = '<div id="perf-fps" style="font-size:18px;font-weight:700;">-- FPS</div>'
      + '<div id="perf-stats" style="margin-top:4px;opacity:0.7;font-size:10px;"></div>';
    ...
    this._tick();
    console.log('[PerfMonitor] 就绪，Ctrl+Alt+P 切换显示');
  },

  toggle() {
    this._enabled = !this._enabled;
    this._el.style.display = this._enabled ? '' : 'none';
    if (this._enabled) {
      this._reset();
      console.log('[PerfMonitor] 已开启');
    } else {
      console.log('[PerfMonitor] 已关闭 | 最低FPS: ' + this._minFps + ' | 掉帧: ' + this._droppedFrames);
    }
  },
  ...
  _tick() {
    ...
    if (frameDelta > this._frameThreshold) {
      this._droppedFrames++;
      this._longFrames.push(Math.round(frameDelta));
      if (this._longFrames.length > this._maxLongFrames) {
        this._longFrames.shift();
      }
    }

    if (now - this._lastTime >= 1000) {
      this._fps = Math.round(this._frames * 1000 / (now - this._lastTime));
      if (this._fps < this._minFps && this._fps > 0) this._minFps = this._fps;
      this._frames = 0;
      this._lastTime = now;
      if (this._enabled) this._updateUI();
    }
  },

  _updateUI() {
    const fpsColor = this._fps >= 55 ? '#4f4' : this._fps >= 30 ? '#fa0' : '#f44';
    document.getElementById('perf-fps').innerHTML =
      '<span style="color:' + fpsColor + '">' + this._fps + '</span> FPS';
    let stats = '最低: ' + (this._minFps === 999 ? '-' : this._minFps) + ' | 掉帧: ' + this._droppedFrames;
    if (this._longFrames.length > 0) {
      stats += '<br>长帧: ' + this._longFrames.slice(-8).join(' ') + 'ms';
    }
    document.getElementById('perf-stats').innerHTML = stats;
  }
};
...
const BoardManager = {
  ...
  _suppressNoteEntrance: false, // 批量刷新便签时抑制入场动画
  // 虚拟坐标系统
  ...
  _selectedConnUnitIndex: -1,
  _connPanelDragging: false,
  _connPanelDragOffset: { x: 0, y: 0 },
  // 端点/控制点拖拽状态
  _draggingEndpoint: null,   // { connId, end, unitIndex }
  _draggingWaypoint: null,   // { connId, wpIndex, unitIndex }
  // 撤回/恢复
  _undoStack: [],
  _redoStack: [],
  _maxUndoSteps: 50,
  ...
  _minimapCache: null,   // 拖动画布时增量更新的缓存
  ...
  init() {
    ...
    // 全局点击关闭连线属性面板
    if (!this._connPanelDocClickBound) {
      this._connPanelDocClickBound = true;
      document.addEventListener('click', (e) => {
        if (!this._selectedConnId) return;
        // 点击面板内部或 SVG 热区 → 不关闭
        const panel = document.getElementById('connPropertyPanel');
        if (panel && panel.contains(e.target)) return;
        if (e.target.closest('.conn-hit-area') || e.target.closest('.waypoint-dot') || e.target.closest('.endpoint-handle')) return;
  ...
  _bindCanvasPan() {
    ...
    document.addEventListener('keyup', (e) => {
      if (e.code === 'Space') {
        ...
        if (this._isCanvasPanning && this._panButton === 0) {
          ...
          const unitCanvas = document.getElementById('unitCanvas_' + this.currentUnitIndex);
          if (unitCanvas) unitCanvas.classList.remove('is-panning');
        }
      }
    });
    ...
    document.addEventListener('pointerdown', (e) => {
      ...
      if (e.button === 2 || this._spacePressed) {
        ...
        // 平移期间抑制便签独立合成层
        const unitCanvas = document.getElementById('unitCanvas_' + this.currentUnitIndex);
        if (unitCanvas) unitCanvas.classList.add('is-panning');
        const vp = this._getUnitViewport();
    ...
    document.addEventListener('wheel', (e) => {
      ...
      this._zoomRafId = requestAnimationFrame(() => {
        ...
        const step = (ShortcutManager._zoomStep ?? 0.02);
  ...
  _setUnitViewport(x, y, scale) {
    ...
    this._updateCanvasTransform();
    // 拖动画布时只更新视口矩形位置，避免每帧重建 minimap DOM
    if (this._isCanvasPanning) {
      this._updateMinimapViewport();
    } else {
      this.renderMinimap();
    }
    this._cullNotes(this.currentUnitIndex);
  },
  ...
  /* 获取视口在虚拟坐标系中的可见矩形（含缓冲区） */
  _getVisibleRect(unitIndex) {
    const idx = unitIndex !== undefined ? unitIndex : this.currentUnitIndex;
    const units = this.getFlowUnits();
    const unit = units[idx];
    const vp = unit && unit.viewport ? unit.viewport : { x: 0, y: 0, scale: 1 };
    ...
    // 获取画布容器的实际像素尺寸
    const unitCanvasEl = document.getElementById('unitCanvas_' + idx);
    ...
    const buffer = 500; // 虚拟像素缓冲区，避免边缘闪烁
    return {
      minX: vp.x - buffer,
      minY: vp.y - buffer,
      maxX: vp.x + viewW + buffer,
      maxY: vp.y + viewH + buffer
    };
  },

  /* 判断便签是否与可见矩形相交 */
  _isNoteVisible(note, visibleRect) {
    const w = note.width || 200;
    const h = note.height || 100;
    return !(note.x + w < visibleRect.minX || note.x > visibleRect.maxX ||
             note.y + h < visibleRect.minY || note.y > visibleRect.maxY);
  },

  /* 视口裁剪：隐藏视口外的便签 DOM 元素 */
  _cullNotes(unitIndex) {
    const canvas = document.getElementById('unitCanvas_' + unitIndex);
    if (!canvas) return;
    const visibleRect = this._getVisibleRect(unitIndex);
    const units = this.getFlowUnits();
    ...
    const elements = canvas.querySelectorAll('.note-card, .plain-text-block, .canvas-text-el, .canvas-image, .battle-deploy-card');
    for (const el of elements) {
      const noteId = el.dataset.noteId;
      if (!noteId) continue;
      const note = noteMap.get(noteId);
      if (!note) continue; // 非便签元素（如战斗部署卡）跳过裁剪
      if (this._isNoteVisible(note, visibleRect)) {
        el.style.visibility = '';
      } else {
  ...
  renderMinimap() {
    ...
    // 缓存关键参数 + 视口矩形 DOM 引用，供 pan 时增量更新复用
    this._minimapCache = {
      minX, minY, scale, viewW, viewH,
      vpEl: canvas.querySelector('.board-minimap-viewport')
    };
  ...
  /* 拖动画布时增量更新鸟瞰图视口矩形，避免每帧 innerHTML 重建 */
  _updateMinimapViewport() {
    const cache = this._minimapCache;
    if (!cache || !cache.vpEl) { this.renderMinimap(); return; }
    ...
    // 容器尺寸可能因窗口 resize 而改变
    const unitCanvasEl = document.getElementById('unitCanvas_' + this.currentUnitIndex);
    if (unitCanvasEl && unitCanvasEl.parentElement) {
      cache.viewW = unitCanvasEl.parentElement.clientWidth / s;
      cache.viewH = unitCanvasEl.parentElement.clientHeight / s;
    }

    cache.vpEl.style.left   = ((vp.x - cache.minX) * cache.scale) + 'px';
    cache.vpEl.style.top    = ((vp.y - cache.minY) * cache.scale) + 'px';
    cache.vpEl.style.width  = (cache.viewW * cache.scale) + 'px';
    cache.vpEl.style.height = (cache.viewH * cache.scale) + 'px';
  },
  ...
  _executeUndo(action) {
    ...
    switch (action.type) {
      ...
      case 'deleteConnection':
        ...
        break;
      case 'editConnection': {
        const editConn = unit.connections.find(c => c.id === action.connId);
        if (editConn && action.oldProps) {
          // 保存当前状态用于恢复
          action.newProps = JSON.parse(JSON.stringify(editConn));
          Object.assign(editConn, action.oldProps);
          if (action.oldProps.lineType !== undefined) editConn.waypoints = action.oldProps.waypoints || [];
          this.renderConnections(unitIndex);
          this._renderConnPanel();
        }
        break;
      }
      case 'batchStyle': {
        for (const ch of action.changes) {
          const n = unit.notes.find(x => x.id === ch.id);
          if (n) this._restoreTextStyle(n, ch.before);
        }
        this.renderUnitNotes(unitIndex);
        this.renderConnections(unitIndex);
        this._refreshSelectionHighlight(unitIndex);
        break;
      }
    }
    this.saveBoard();
  ...
  _executeRedo(action) {
    ...
    switch (action.type) {
      ...
      case 'deleteConnection':
        ...
        break;
      case 'editConnection': {
        const redoConn = unit.connections.find(c => c.id === action.connId);
        if (redoConn && action.newProps) {
          const swapProps = JSON.parse(JSON.stringify(redoConn));
          Object.assign(redoConn, action.newProps);
          action.newProps = swapProps;
          this.renderConnections(unitIndex);
          this._renderConnPanel();
        }
        break;
      ...
      case 'batchStyle': {
        for (const ch of action.changes) {
          const n = unit.notes.find(x => x.id === ch.id);
          if (n) this._restoreTextStyle(n, ch.after);
        }
        this.renderUnitNotes(unitIndex);
        this.renderConnections(unitIndex);
        this._refreshSelectionHighlight(unitIndex);
  ...
  renderNote(note, container, unitIndex) {
    ...
    el.addEventListener('pointerdown', (e) => {
      ...
      if (e.target.closest('.dice-range-header')) return;
    ...
    // 触发入场弹入动画（批量刷新时跳过，避免原地重建触发弹跳）
    if (!this._suppressNoteEntrance) {
  ...
  _normalizeConn(conn) {
    if (!conn) return;
    const defaults = {
      color: '#c0ab84', lineType: 'straight', lineWidth: 2, lineStyle: 'solid',
      startArrow: 'none', endArrow: 'none', arrowSize: 8,
      waypoints: [], fromAnchor: null, toAnchor: null
    };
    Object.keys(defaults).forEach(k => {
      if (conn[k] === undefined || conn[k] === null) conn[k] = defaults[k];
    });
    // 确保 waypoints 是数组
    if (!Array.isArray(conn.waypoints)) conn.waypoints = [];
    // 为旧连接生成唯一 ID（向后兼容）
    if (!conn.id) conn.id = `c_legacy_${conn.from}_${conn.to}_${Math.random().toString(36).slice(2, 7)}`;
  },
  ...
  // ========== 边缘锚定函数 ==========
  _lineRectIntersection(fromPt, toPt, l, t, r, b) {
    const dx = toPt.x - fromPt.x, dy = toPt.y - fromPt.y;
    if (dx === 0 && dy === 0) return null;
    let tMin = Infinity, best = null;
    if (dx !== 0) {
      let te = (l - fromPt.x) / dx, y = fromPt.y + te * dy;
      if (te >= 0 && y >= t && y <= b && te < tMin) { tMin = te; best = { x: l, y }; }
      te = (r - fromPt.x) / dx; y = fromPt.y + te * dy;
      if (te >= 0 && y >= t && y <= b && te < tMin) { tMin = te; best = { x: r, y }; }
    }
    if (dy !== 0) {
      let te = (t - fromPt.y) / dy, x = fromPt.x + te * dx;
      if (te >= 0 && x >= l && x <= r && te < tMin) { tMin = te; best = { x, y: t }; }
      te = (b - fromPt.y) / dy; x = fromPt.x + te * dx;
      if (te >= 0 && x >= l && x <= r && te < tMin) { tMin = te; best = { x, y: b }; }
    }
    return best;
  },

  _anchorToPoint(rect, anchor) {
    if (!anchor) return { x: rect.x + rect.w / 2, y: rect.y + rect.h / 2 };
    const { x, y, w, h } = rect;
    switch (anchor.side) {
      case 'top': return { x: x + w * anchor.offset, y };
      case 'bottom': return { x: x + w * anchor.offset, y: y + h };
      case 'left': return { x, y: y + h * anchor.offset };
      case 'right': return { x: x + w, y: y + h * anchor.offset };
      default: return { x: x + w / 2, y: y + h / 2 };
    }
  },

  _calcEdgePoint(rect, anchor, otherCenter) {
    if (anchor) return this._anchorToPoint(rect, anchor);
    const cx = rect.x + rect.w / 2, cy = rect.y + rect.h / 2;
    return this._lineRectIntersection(
      { x: cx, y: cy }, otherCenter,
      rect.x, rect.y, rect.x + rect.w, rect.y + rect.h
    ) || { x: cx, y: cy };
  },

  _snapToCardEdge(px, py, rect) {
    const l = rect.x, r = rect.x + rect.w, t = rect.y, b = rect.y + rect.h;
    const d = [Math.abs(py - t), Math.abs(py - b), Math.abs(px - l), Math.abs(px - r)];
    const m = Math.min(...d);
    if (m === d[0]) { const o = Math.max(0, Math.min(1, (px - l) / rect.w)); return { side: 'top', offset: o, x: l + rect.w * o, y: t }; }
    if (m === d[1]) { const o = Math.max(0, Math.min(1, (px - l) / rect.w)); return { side: 'bottom', offset: o, x: l + rect.w * o, y: b }; }
    if (m === d[2]) { const o = Math.max(0, Math.min(1, (py - t) / rect.h)); return { side: 'left', offset: o, x: l, y: t + rect.h * o }; }
    const o = Math.max(0, Math.min(1, (py - t) / rect.h)); return { side: 'right', offset: o, x: r, y: t + rect.h * o };
  },

  // ========== 连线路径计算 ==========
  _calcStraightPath(c1, c2) { return { d: `M ${c1.x} ${c1.y} L ${c2.x} ${c2.y}`, waypoints: [] }; },

  _calcCurvedPath(c1, c2, conn) {
    if (conn.waypoints && conn.waypoints.length >= 2) {
      const cp1 = conn.waypoints[0], cp2 = conn.waypoints[1];
    ...
    const cp1 = { x: c1.x + dx * 0.3, y: c1.y + dy * 0.3 + (Math.abs(dx) > Math.abs(dy) ? off * 0.5 : 0) };
    const cp2 = { x: c2.x - dx * 0.3, y: c2.y - dy * 0.3 - (Math.abs(dx) > Math.abs(dy) ? off * 0.5 : 0) };
    return { d: `M ${c1.x} ${c1.y} C ${cp1.x} ${cp1.y}, ${cp2.x} ${cp2.y}, ${c2.x} ${c2.y}`, waypoints: [cp1, cp2] };
  },

  _calcOrthogonalPath(c1, c2, conn) {
    if (conn.waypoints && conn.waypoints.length >= 1) {
      const pts = [c1, ...conn.waypoints, c2];
      return { d: pts.map((p, i) => i === 0 ? `M ${p.x} ${p.y}` : `L ${p.x} ${p.y}`).join(' '), waypoints: conn.waypoints };
    }
  ...
  _calcConnectionPath(c1, c2, conn) {
    switch (conn.lineType) {
      case 'curved': return this._calcCurvedPath(c1, c2, conn);
      case 'orthogonal': return this._calcOrthogonalPath(c1, c2, conn);
      default: return this._calcStraightPath(c1, c2);
    }
  },
  ...
  // ========== 从 DOM 获取元素边界矩形（虚拟坐标） ==========
  _getElRect(unitIndex, noteId) {
    const el = document.querySelector(`#unitCanvas_${unitIndex} [data-note-id="${noteId}"]`);
    // display:none（视口裁剪）时 offset 属性全为 0，跳过 DOM 读取，走数据层回退
    if (el && !(el.offsetWidth === 0 && el.offsetHeight === 0)) {
      let x = el.offsetLeft, y = el.offsetTop;
      // 拖拽时元素通过 transform: translate 定位，offsetLeft/Top 不包含 transform，需手动补偿
    ...
    const unit = units[unitIndex];
    if (!unit) return null;
    const note = unit.notes.find(n => n.id === noteId) || this._getBattle(noteId);
    if (!note) return null;
    return { x: note.x || 0, y: note.y || 0, w: note.w || note.width || 200, h: note.h || note.height || 100 };
  },
  ...
  renderConnections(unitIndex) {
    ...
    let defsHtml = `<filter id="glow"><feGaussianBlur stdDeviation="2.5" result="coloredBlur"/><feMerge><feMergeNode in="coloredBlur"/><feMergeNode in="SourceGraphic"/></feMerge></filter>`;
    let connsHtml = '';
    let waypointsHtml = '';
    let endpointsHtml = '';

    unit.connections.forEach((conn, idx) => {
      this._normalizeConn(conn); // 向后兼容：补全缺失字段

      const fromRect = this._getElRect(unitIndex, conn.from);
      const toRect = this._getElRect(unitIndex, conn.to);
      if (!fromRect || !toRect) { return; }

      const fc = { x: fromRect.x + fromRect.w / 2, y: fromRect.y + fromRect.h / 2 };
      const tc = { x: toRect.x + toRect.w / 2, y: toRect.y + toRect.h / 2 };
      const c1 = this._calcEdgePoint(fromRect, conn.fromAnchor, tc);
      const c2 = this._calcEdgePoint(toRect, conn.toAnchor, fc);

      // 遗留连线：首次渲染时自动计算并固定端点锚定，之后拖动元素位置不变
      if (!conn.fromAnchor) conn.fromAnchor = this._snapToCardEdge(c1.x, c1.y, fromRect);
      if (!conn.toAnchor) conn.toAnchor = this._snapToCardEdge(c2.x, c2.y, toRect);

      const pd = this._calcConnectionPath(c1, c2, conn);
      if (!pd.d) return;

      const isSelected = conn.id === this._selectedConnId && unitIndex === this._selectedConnUnitIndex;

      // 箭头 marker defs
      const arrowSize = conn.arrowSize || 8;
      ['start', 'end'].forEach(pos => {
        const type = pos === 'start' ? conn.startArrow : conn.endArrow;
        if (!type || type === 'none') return;
        const tdef = this._ARROW_DEFS[type];
        if (!tdef) return;
        const { shape, attrs } = tdef.create(arrowSize, conn.color, pos === 'start');
        const mid = `arrow-${conn.id}-${pos}`;
        defsHtml += `<marker id="${mid}" markerWidth="${arrowSize+2}" markerHeight="${arrowSize}" refX="${tdef.refX(arrowSize)}" refY="${arrowSize/2}" orient="auto" markerUnits="userSpaceOnUse">`;
        defsHtml += `<${shape} ${Object.entries(attrs).map(([k,v])=>`${k}="${v}"`).join(' ')}/>`;
        defsHtml += `</marker>`;
      });
      ...
      let strokeVal = conn.color;
      if (fromIsText || toIsText) {
        const gid = `connGrad_${unitIndex}_${idx}`;
        let grad = `<linearGradient id="${gid}" gradientUnits="userSpaceOnUse" x1="${c1.x}" y1="${c1.y}" x2="${c2.x}" y2="${c2.y}">`;
        grad += fromIsText ? `<stop offset="0%" stop-color="${conn.color}" stop-opacity="0"/><stop offset="30%" stop-color="${conn.color}" stop-opacity="1"/>` : `<stop offset="0%" stop-color="${conn.color}" stop-opacity="1"/>`;
        grad += toIsText ? `<stop offset="70%" stop-color="${conn.color}" stop-opacity="1"/><stop offset="100%" stop-color="${conn.color}" stop-opacity="0"/>` : `<stop offset="100%" stop-color="${conn.color}" stop-opacity="1"/>`;
        grad += `</linearGradient>`;
        defsHtml += grad;
        strokeVal = `url(#${gid})`;
      }
      ...
      let dashAttr = '';
      let linecapAttr = '';
      if (conn.lineStyle === 'dashed') dashAttr = `stroke-dasharray="${8 * conn.lineWidth} ${4 * conn.lineWidth}"`;
      else if (conn.lineStyle === 'dotted') { dashAttr = `stroke-dasharray="${2 * conn.lineWidth} ${4 * conn.lineWidth}"`; linecapAttr = 'stroke-linecap="round"'; }

      // 箭头 marker 引用
      const markerEnd = (conn.endArrow && conn.endArrow !== 'none') ? `marker-end="url(#arrow-${conn.id}-end)"` : '';
      const markerStart = (conn.startArrow && conn.startArrow !== 'none') ? `marker-start="url(#arrow-${conn.id}-start)"` : '';

      // 连线组（可见路径 + 透明热区 + 发光层）
      connsHtml += `<g class="conn-group-item${isSelected ? ' selected' : ''}" data-conn-id="${conn.id}">`;
      // 发光层
      connsHtml += `<path d="${pd.d}" stroke="${conn.color}" stroke-width="${conn.lineWidth}" fill="none" ${linecapAttr} filter="url(#glow)" opacity="0.15" style="pointer-events:none;"/>`;
      // 可见路径
      connsHtml += `<path d="${pd.d}" stroke="${strokeVal}" stroke-width="${conn.lineWidth}" fill="none" ${linecapAttr} ${dashAttr} ${markerEnd} ${markerStart} class="conn-path-vis"/>`;
      // 透明宽热区（SVG 原生 pointer-events 属性，可突破父级 CSS pointer-events:none）
      connsHtml += `<path d="${pd.d}" stroke="transparent" stroke-width="14" fill="none" pointer-events="stroke" class="conn-hit-area" data-conn-idx="${idx}" data-unit-idx="${unitIndex}"/>`;
      connsHtml += `</g>`;

      // 控制点（仅选中时）
      if (isSelected && pd.waypoints && pd.waypoints.length > 0) {
        pd.waypoints.forEach((wp, wi) => {
          waypointsHtml += `<circle cx="${wp.x}" cy="${wp.y}" r="5" class="waypoint-dot visible" data-conn-id="${conn.id}" data-wp-idx="${wi}" data-unit-idx="${unitIndex}"/>`;
        });
      }
      // 端点手柄（仅选中时）
      if (isSelected) {
        endpointsHtml += `<circle cx="${c1.x}" cy="${c1.y}" r="6" class="endpoint-handle visible" data-conn-id="${conn.id}" data-end="from" data-unit-idx="${unitIndex}"/>`;
        endpointsHtml += `<circle cx="${c2.x}" cy="${c2.y}" r="6" class="endpoint-handle visible" data-conn-id="${conn.id}" data-end="to" data-unit-idx="${unitIndex}"/>`;
      }
    });

    svg.innerHTML = `<defs>${defsHtml}</defs><g class="conn-group">${connsHtml}</g><g class="waypoint-group">${waypointsHtml}</g><g class="endpoint-group">${endpointsHtml}</g>`;

    // 绑定事件
    this._bindConnEvents(svg);
  },

  _bindConnEvents(svg) {
    // 热区点击 → 选中连线或擦除
    svg.querySelectorAll('.conn-hit-area').forEach(el => {
      // capture 阶段 mousedown/pointerdown/touchstart 阻止画布拖拽/框选劫持
      const stopDrag = (e) => { e.stopPropagation(); e.stopImmediatePropagation(); };
      el.addEventListener('mousedown', stopDrag, true);
      el.addEventListener('pointerdown', stopDrag, true);
      el.addEventListener('touchstart', stopDrag, true);
      el.addEventListener('click', (e) => {
        e.stopPropagation();
        const ui = parseInt(el.dataset.unitIdx);
        const ci = parseInt(el.dataset.connIdx);
        this._onConnectionClick(ui, ci);
      });
    });
    // 控制点拖拽（capture 阶段阻止画布框选）
    svg.querySelectorAll('.waypoint-dot').forEach(el => {
      el.addEventListener('mousedown', (e) => {
        e.stopPropagation(); e.stopImmediatePropagation();
        this._onWaypointDown(e, el.dataset.connId, parseInt(el.dataset.wpIdx), parseInt(el.dataset.unitIdx));
      }, true);
    ...
    // 端点拖拽（capture 阶段阻止画布框选）
    svg.querySelectorAll('.endpoint-handle').forEach(el => {
      el.addEventListener('mousedown', (e) => {
        e.stopPropagation(); e.stopImmediatePropagation();
        this._onEndpointDown(e, el.dataset.connId, el.dataset.end, parseInt(el.dataset.unitIdx));
      }, true);
      el.addEventListener('pointerdown', (e) => { e.stopPropagation(); e.stopImmediatePropagation(); }, true);
      el.addEventListener('touchstart', (e) => { e.stopPropagation(); e.stopImmediatePropagation(); }, true);
      el.addEventListener('click', (e) => { e.stopPropagation(); });
  ...
  _onWaypointDown(e, connId, wpIdx, unitIndex) {
    if (e.button !== 0) return;
    const units = this.getFlowUnits();
    const unit = units[unitIndex];
    if (!unit) return;
    const conn = unit.connections.find(c => c.id === connId);
    if (!conn) return;
    // 确保 waypoints 数组存在
    if (!conn.waypoints || conn.waypoints.length === 0) {
      ...
      const c2 = this._calcEdgePoint(toRect, conn.toAnchor, fc);
      const pd = this._calcConnectionPath(c1, c2, conn);
      if (pd.waypoints && pd.waypoints.length > 0) {
        conn.waypoints = pd.waypoints.map(w => ({ x: w.x, y: w.y }));
      }
    }
    this._draggingWaypoint = { connId, wpIndex: wpIdx, unitIndex };
    e.preventDefault();
  },

  _onEndpointDown(e, connId, end, unitIndex) {
    if (e.button !== 0) return;
    this._draggingEndpoint = { connId, end, unitIndex };
    e.preventDefault();
  },
  ...
  _renderPlainTextBlock(note, container, unitIndex) {
    ...
    el.addEventListener('pointerdown', (e) => {
      ...
      if (e.target.closest('.pt-delete-btn')) return;
  ...
  _renderTextElement(note, container, unitIndex) {
    ...
    // 删除按钮（hover时显示）
    el.insertAdjacentHTML('beforeend', `<div class="pt-delete-btn" onclick="event.stopPropagation(); BoardManager.deleteTextElement('${note.id}', ${unitIndex})" title="删除"><svg viewBox="0 0 24 24" width="14" height="14" style="stroke:currentColor;fill:none;stroke-width:2;stroke-linecap:round;stroke-linejoin:round;"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a2 2 0 012-2h4a2 2 0 012 2v2"/></svg></div>`);
  ...
  editNote(noteId, unitIndex) {
    ...
    if (note.type === 'characters') {
      ...
      bodyHtml += `<div id="editEncRangesList" style="max-height:300px;overflow-y:auto;">`;
      dr.forEach((r, i) => {
        bodyHtml += `<div class="enc-range-row" data-idx="${i}" style="display:flex;gap:6px;margin-bottom:6px;align-items:flex-start;">`;
        bodyHtml += `<div style="display:flex;gap:4px;align-items:center;flex-shrink:0;padding-top:4px;">`;
        bodyHtml += `<input type="text" class="enc-range-min" value="${this._esc(String(r.min || 1))}" style="width:45px;text-align:center;border:1px solid var(--border);border-radius:4px;padding:4px;font-size:12px;box-sizing:border-box;">`;
        bodyHtml += `<span style="color:var(--text-muted);">~</span>`;
        bodyHtml += `<input type="text" class="enc-range-max" value="${this._esc(String(r.max || 5))}" style="width:45px;text-align:center;border:1px solid var(--border);border-radius:4px;padding:4px;font-size:12px;box-sizing:border-box;">`;
        bodyHtml += `</div>`;
        bodyHtml += `<textarea class="enc-range-content" style="flex:1;border:1px solid var(--border);border-radius:4px;padding:4px;font-size:12px;box-sizing:border-box;min-height:46px;resize:vertical;">${this._esc(r.content || '')}</textarea>`;
        bodyHtml += `<button onclick="this.parentElement.remove()" style="border:none;background:none;color:var(--text-muted);cursor:pointer;padding:4px;flex-shrink:0;">✕</button>`;
        bodyHtml += `</div>`;
      });
      bodyHtml += `</div>`;
      bodyHtml += `<button onclick="BoardManager._addEditEncRangeRow()" type="button" style="padding:4px 8px;font-size:11px;border:1px dashed var(--border);border-radius:4px;background:none;color:var(--text-muted);cursor:pointer;margin-top:4px;">+ 添加范围</button>`;
    ...
    modal.innerHTML = `
      <div class="board-edit-modal-content">
        ...
        <div class="board-edit-body">
          ${note.type !== 'characters' ? `<label>标题</label><input type="text" id="editNoteTitle" value="${this._esc(note.title || '')}" placeholder="便签标题" style="border:1px solid var(--border);border-radius:4px;padding:4px 6px;font-size:12px;box-sizing:border-box;">` : ''}
          ${bodyHtml}
        </div>
        <div class="board-edit-footer">
          <button onclick="BoardManager.closeEditModal()">取消</button>
  ...
  editNoteTitle(noteId, unitIndex) {
    console.log('[NoteEdit] editNoteTitle 进入', { noteId, unitIndex, hasPrevEdit: !!this._editingNote, prevMode: this._editingNote ? this._editingNote.mode : null });
    ...
    console.log('[NoteEdit] editNoteTitle 入口', { noteId, unitIndex, hasOldEditing: !!this._editingNote, oldEditing: this._editingNote ? JSON.stringify(this._editingNote) : null });
    ...
    const finishEdit = (saveFlag) => {
      console.log('[NoteEdit] finishEdit 入口', { saveFlag, hasEditingNote: !!this._editingNote, editingMode: this._editingNote ? this._editingNote.mode : null, inputInDOM: document.body.contains(input) });
      ...
      console.log('[NoteEdit] finishEdit 完成', { noteId, saveFlag, restoredTitle: newTitle });
    };

    const clickOutsideHandler = (e) => {
      const contains = el.contains(e.target);
      console.log('[NoteEdit] clickOutsideHandler', { target: e.target.tagName + '.' + (e.target.className || ''), contains, hasEditingNote: !!this._editingNote, editingMode: this._editingNote ? this._editingNote.mode : null });
    ...
    input.addEventListener('blur', () => {
      console.log('[NoteEdit] blur 触发', { noteId, hasEditingNote: !!this._editingNote });
      setTimeout(() => {
        const stillEditing = this._editingNote && this._editingNote.mode === 'title';
        console.log('[NoteEdit] blur setTimeout 回调', { noteId, hasEditingNote: !!this._editingNote, editingMode: this._editingNote ? this._editingNote.mode : null, stillEditing });
        if (stillEditing) {
          finishEdit(true);
        }
      }, 10);
  ...
  _exitNoteEdit() {
    ...
    console.log('[NoteEdit] _exitNoteEdit 被调用', { noteId: this._editingNote.noteId, mode: this._editingNote.mode, stack: new Error().stack.split('\n').slice(1,4).map(s => s.trim()) });
  ...
  saveEditModal(noteId, unitIndex) {
    ...
    if (note.type === 'characters') {
      ...
      // 从表单收集遭遇骰点范围数据
      const ranges = [];
      document.querySelectorAll('#editEncRangesList .enc-range-row').forEach(row => {
        const minEl = row.querySelector('.enc-range-min');
        const maxEl = row.querySelector('.enc-range-max');
        const contentEl = row.querySelector('.enc-range-content');
        const min = parseInt(minEl ? minEl.value.trim() : '1') || 1;
        const max = parseInt(maxEl ? maxEl.value.trim() : '5') || 5;
        const content = contentEl ? contentEl.value.trim() : '';
        ranges.push({ min, max, content });
      });
      note.diceRanges = ranges;
  ...
  _onGlobalPointerMove(e) {
    if (AppState.currentPage !== 'board') return;
    // RAF 节流：pointermove 高频触发，只缓存最新事件，每帧统一处理一次
  ...
  _processPointerMove(e) {
    ...
    if (this._draggingWaypoint) {
      const canvas = document.getElementById('unitCanvas_' + this._draggingWaypoint.unitIndex);
      if (!canvas) return;
      const cr = canvas.getBoundingClientRect();
      const vp = this._screenToVirtual(e.clientX - cr.left, e.clientY - cr.top, this._draggingWaypoint.unitIndex);
      const units = this.getFlowUnits();
      const unit = units[this._draggingWaypoint.unitIndex];
      if (unit) {
        const conn = unit.connections.find(c => c.id === this._draggingWaypoint.connId);
        if (conn && conn.waypoints) {
          conn.waypoints[this._draggingWaypoint.wpIndex] = { x: vp.x, y: vp.y };
        }
      }
      this.renderConnections(this._draggingWaypoint.unitIndex);
      return;
    }

    // 端点拖拽
    if (this._draggingEndpoint) {
      const canvas = document.getElementById('unitCanvas_' + this._draggingEndpoint.unitIndex);
      if (!canvas) return;
      const cr = canvas.getBoundingClientRect();
      const vp = this._screenToVirtual(e.clientX - cr.left, e.clientY - cr.top, this._draggingEndpoint.unitIndex);
      const units = this.getFlowUnits();
      const unit = units[this._draggingEndpoint.unitIndex];
      if (unit) {
        const conn = unit.connections.find(c => c.id === this._draggingEndpoint.connId);
        if (conn) {
          const noteId = this._draggingEndpoint.end === 'from' ? conn.from : conn.to;
          const rect = this._getElRect(this._draggingEndpoint.unitIndex, noteId);
          if (rect) {
            const snap = this._snapToCardEdge(vp.x, vp.y, rect);
            if (this._draggingEndpoint.end === 'from') conn.fromAnchor = { side: snap.side, offset: snap.offset };
            else conn.toAnchor = { side: snap.side, offset: snap.offset };
          }
        }
      }
      this.renderConnections(this._draggingEndpoint.unitIndex);
      return;
    }

    // 连线属性面板拖拽
    if (this._connPanelDragging) {
      const panel = document.getElementById('connPropertyPanel');
      if (panel) {
        panel.style.left = (e.clientX - this._connPanelDragOffset.x) + 'px';
        panel.style.top = (e.clientY - this._connPanelDragOffset.y) + 'px';
      }
      return;
    }
    ...
    if (this._isMultiDrag && this._multiDragNotes.length > 0) {
      ...
      for (const item of this._multiDragNotes) {
        ...
        if (el) {
          const tdx = note.x - item.startX;
          const tdy = note.y - item.startY;
          const scalePart = el.classList.contains('note-card') ? ' scale(1.02)' : '';
          el.style.transform = `translate(${tdx}px, ${tdy}px)${scalePart}`;
        }
      }
      ...
      if (noteEl) {
        const tdx = this.dragNote.x - this._dragStartVirtual.x;
        const tdy = this.dragNote.y - this._dragStartVirtual.y;
        const scalePart = noteEl.classList.contains('note-card') ? ' scale(1.02)' : '';
        noteEl.style.transform = `translate(${tdx}px, ${tdy}px)${scalePart}`;
      }
      this.renderConnections(this.dragUnitIndex);
      this.renderMinimap();
    }
  ...
  _onGlobalPointerUp(e) {
    if (AppState.currentPage !== 'board') return;
    const wasMultiDrag = this._isMultiDrag;
    // 清除多选拖拽状态
    if (this._isMultiDrag) {
      for (const item of this._multiDragNotes) {
        const mel = document.querySelector(`#unitCanvas_${this.dragUnitIndex} [data-note-id="${item.noteId}"]`);
        if (mel) {
          mel.style.left = item.note.x + 'px';
          mel.style.top = item.note.y + 'px';
          mel.style.transform = '';
  ...
  /* 判断元素是否支持统一修改文字颜色/字号 */
  _isStyleableNote(note) {
    if (!note) return false;
    if (note.type === 'text' || note.type === 'plaintext') return true;
    // 排除角色便签、图片、骰子便签、背景框等非纯文本元素，其余 note-card 视为通用便签
    if (note.type === 'characters' || note.type === 'image' || note.type === 'encounters' || note.type === 'blindbox' || note.type === 'background_frame') return false;
    return true;
  },

  /* 快照一个便签的文字样式（text / plaintext / 通用便签三种存储位置） */
  _snapshotTextStyle(note) {
    const s = note.style || {};
    return {
      textFontSize: note.fontSize,     // text 类型
      textColor: note.color,
      ptFontSize: s.fontSize,          // plaintext 类型
      ptColor: s.color,
      content: note.content            // 通用便签：快照正文 HTML
    };
  },

  /* 按便签类型恢复文字样式快照 */
  _restoreTextStyle(note, snap) {
    if (!note || !snap) return;
    if (note.type === 'text') {
      note.fontSize = snap.textFontSize;
      note.color = snap.textColor;
    } else if (note.type === 'plaintext') {
      note.style = note.style || {};
      note.style.fontSize = snap.ptFontSize;
      note.style.color = snap.ptColor;
    } else {
      note.content = snap.content;
    }
  },

  /* 把颜色/字号烘焙进便签正文 HTML（覆盖旧的局部色号，保留加粗/斜体等格式） */
  _applyStyleToNoteHtml(content, color, fontSize) {
    if (!content) return '';
    const tmp = document.createElement('div');
    tmp.innerHTML = content;

    const els = tmp.querySelectorAll('*');
    if (els.length === 0) {
      // 纯文本内容：包一层带样式的 span，保证颜色/字号有落点
      const span = document.createElement('span');
  ...
  /* 重渲染后恢复选中高亮（renderUnitNotes 会重建 DOM，丢失 .selected） */
  _refreshSelectionHighlight(unitIndex) {
    const idx = unitIndex !== undefined ? unitIndex : this.currentUnitIndex;
    const canvas = document.getElementById('unitCanvas_' + idx);
    if (!canvas) return;
    canvas.querySelectorAll('.note-card, .plain-text-block, .canvas-text-el, .canvas-image').forEach(el => {
  ...
  /* 统一设置选中元素的文字颜色/字号（color 或 fontSize 为 null 表示不改该项） */
  applyTextStyleToSelection(color, fontSize) {
    ...
    for (const id of this._selectedNoteIds) {
      ...
      if (!this._isStyleableNote(note)) continue;

      const before = this._snapshotTextStyle(note);

      if (note.type === 'text') {
        if (fontSize != null) note.fontSize = fontSize;
        if (color != null) note.color = color;
      } else if (note.type === 'plaintext') {
        note.style = note.style || {};
        if (fontSize != null) note.style.fontSize = fontSize;
        if (color != null) note.style.color = color;
      } else {
        note.content = this._applyStyleToNoteHtml(note.content, color, fontSize);
      }

      const after = this._snapshotTextStyle(note);
      changes.push({ id, before, after });
    }

    if (changes.length === 0) return;

    this._pushUndo({ type: 'batchStyle', unitIndex: this.currentUnitIndex, changes });

    this._suppressNoteEntrance = true;
    try {
      this.renderUnitNotes(this.currentUnitIndex);
      this.renderConnections(this.currentUnitIndex);
      this._refreshSelectionHighlight();
    } finally {
      this._suppressNoteEntrance = false;
    }
    requestAnimationFrame(() => this._updateMultiSelectToolbar());
    this.saveBoard();
    StorageManager.scheduleSave();
  },
  ...
  /* 获取（首次创建）浮动工具条 DOM */
  _ensureMultiSelectToolbar() {
    if (this._multiSelectToolbar) return this._multiSelectToolbar;
    ...
    bar.innerHTML = `
      <span class="mst-count" id="mstCount">0 项</span>
      <span class="mst-sep"></span>
      <div class="mst-color-wrap">
        <button class="mst-color-btn" id="mstColorBtn" title="文字颜色" type="button">
          <span class="mst-color-swatch" id="mstColorSwatch"></span>
          <span class="mst-caret">▾</span>
        </button>
        <div class="mst-color-picker" id="mstColorPicker" style="display:none;">
          <div class="mst-color-section">
            <div class="mst-color-title">主题颜色</div>
            <div class="mst-color-grid" id="mstThemeColors">
              ${themeColors.map(c => `<div class="mst-color-cell" data-color="${c}" style="background-color:${c};"></div>`).join('')}
            </div>
          </div>
          <div class="mst-color-row">
            <input type="text" class="mst-hex-input" id="mstHexInput" placeholder="#FF0000">
            <input type="color" class="mst-sys-picker" id="mstSysPicker" title="系统取色器">
          </div>
        </div>
      </div>
      <span class="mst-sep"></span>
      <input type="number" class="mst-fontsize-input" id="mstFontSizeInput" min="8" max="72" step="1" placeholder="字号">
      <span class="mst-unit">px</span>
    `;
    document.body.appendChild(bar);

    this._bindMultiSelectToolbar(bar);
    this._multiSelectToolbar = bar;
    return bar;
  },

  /* 绑定浮动工具条事件 */
  _bindMultiSelectToolbar(bar) {
    const colorBtn = bar.querySelector('#mstColorBtn');
    const picker = bar.querySelector('#mstColorPicker');
    const swatch = bar.querySelector('#mstColorSwatch');
    const hexInput = bar.querySelector('#mstHexInput');
    const sysPicker = bar.querySelector('#mstSysPicker');
    const fontSizeInput = bar.querySelector('#mstFontSizeInput');
    ...
    bar.querySelectorAll('.mst-color-cell').forEach(cell => {
      cell.addEventListener('mousedown', (e) => {
        e.preventDefault();
        e.stopPropagation();
        const color = cell.getAttribute('data-color');
        this._applyMultiSelectColor(color);
        picker.style.display = 'none';
      });
    });

    sysPicker.addEventListener('change', (e) => {
      e.stopPropagation();
      const color = e.target.value;
    ...
    sysPicker.addEventListener('mousedown', (e) => e.stopPropagation());
    ...
    fontSizeInput.addEventListener('change', (e) => {
      e.stopPropagation();
      this._applyMultiSelectFontSize(e.target.value);
    });

    // 点击工具条外部关闭颜色面板
    if (!this._mstDocClickBound) {
      this._mstDocClickBound = true;
      document.addEventListener('mousedown', (e) => {
        const b = this._multiSelectToolbar;
        if (!b) return;
        const p = b.querySelector('#mstColorPicker');
        if (p && p.style.display !== 'none' && !b.contains(e.target)) {
  ...
  /* 更新浮动工具条位置/显隐（基于选中元素包围盒） */
  _updateMultiSelectToolbar() {
    const bar = this._ensureMultiSelectToolbar();
    const units = this.getFlowUnits();
    const unit = units[this.currentUnitIndex];
    const canvas = document.getElementById('unitCanvas_' + this.currentUnitIndex);
    if (!unit || !canvas) {
    ...
    this._selectedNoteIds.forEach(id => {
      const note = unit.notes.find(n => n.id === id);
      if (note && this._isStyleableNote(note)) styleable.push(note);
    });
    ...
    bar.querySelector('#mstCount').textContent = styleable.length + ' 项';
    bar.style.display = 'flex';
    bar.style.visibility = 'hidden'; // 先隐藏以测量尺寸

    const barRect = bar.getBoundingClientRect();
    let left = (minLeft + maxRight) / 2 - barRect.width / 2;
    let top = minTop - barRect.height - 10;
    if (top < 8) top = maxBottom + 10;

    left = Math.max(8, Math.min(left, window.innerWidth - barRect.width - 8));
    top = Math.max(8, Math.min(top, window.innerHeight - barRect.height - 8));
  ...
  /* 隐藏浮动工具条 */
  _hideMultiSelectToolbar() {
    const bar = this._multiSelectToolbar;
    if (!bar) return;
    bar.style.display = 'none';
    const picker = bar.querySelector('#mstColorPicker');
    if (picker) picker.style.display = 'none';
  ...
  /* 应用颜色到选中元素 */
  _applyMultiSelectColor(color) {
    if (!color) return;
    this.applyTextStyleToSelection(color, null);
    const bar = this._multiSelectToolbar;
    if (bar) {
      const swatch = bar.querySelector('#mstColorSwatch');
  ...
  /* 应用字号到选中元素 */
  _applyMultiSelectFontSize(value) {
    const n = parseInt(value, 10);
    if (!isFinite(n) || n < 8 || n > 72) return;
    this.applyTextStyleToSelection(null, n);
  },
  ...
  deleteConnection(unitIndex, connIndex) {
    ...
    // 如果删除的是当前选中的连线，清除选中状态
    if (conn && this._selectedConnId === conn.id) {
      this._selectedConnId = null;
      this._selectedConnUnitIndex = -1;
      this._hideConnPanel();
    }
    this.renderConnections(unitIndex);
    this.saveBoard();
  ...
  _onConnectionClick(unitIndex, connIndex) {
    ...
    const units = this.getFlowUnits();
    const unit = units[unitIndex];
    if (!unit || !unit.connections[connIndex]) { return; }
    const conn = unit.connections[connIndex];
    // 已选中的连线再点击 → 取消选中
    if (this._selectedConnId === conn.id && this._selectedConnUnitIndex === unitIndex) {
      this._clearConnSelection();
      return;
    }
    this._selectedConnId = conn.id;
    this._selectedConnUnitIndex = unitIndex;
    this.renderConnections(unitIndex);
    this._renderConnPanel(unitIndex);
    this._positionConnPanel();
  },

  _clearConnSelection() {
    const prevUnitIndex = this._selectedConnUnitIndex;
    this._selectedConnId = null;
    this._selectedConnUnitIndex = -1;
    const panel = document.getElementById('connPropertyPanel');
    if (panel) panel.classList.remove('visible');
    // 重新渲染当前单元连线以移除选中状态
    if (prevUnitIndex >= 0) {
      this.renderConnections(prevUnitIndex);
    }
  },

  // ========== 连线属性面板 ==========
  _initConnPanelColors() {
    const container = document.getElementById('cpColorSwatches');
    if (!container) return;
    this.CONNECTION_COLORS.forEach(cl => {
      const sw = document.createElement('div');
      sw.className = 'cp-color-swatch';
      sw.style.backgroundColor = cl;
      sw.dataset.color = cl;
      sw.title = cl;
      sw.onclick = () => this._cpSetColor(cl);
      container.appendChild(sw);
    });
  },

  _getSelectedConn() {
    if (!this._selectedConnId || this._selectedConnUnitIndex < 0) return null;
    const units = this.getFlowUnits();
    const unit = units[this._selectedConnUnitIndex];
    if (!unit) return null;
    this._normalizeConn(unit.connections.find(c => c.id === this._selectedConnId));
    return unit.connections.find(c => c.id === this._selectedConnId);
  },

  _getSelectedUnit() {
    const units = this.getFlowUnits();
    return units[this._selectedConnUnitIndex] || null;
  },

  _renderConnPanel(unitIndex) {
    const conn = this._getSelectedConn();
    if (!conn) { this._hideConnPanel(); return; }
    const panel = document.getElementById('connPropertyPanel');
    if (!panel) return;
    panel.classList.add('visible');

    // 线型按钮状态
    panel.querySelectorAll('.cp-type-btn').forEach(b => b.classList.toggle('active', b.dataset.type === conn.lineType));
    // 颜色
    panel.querySelectorAll('.cp-color-swatch').forEach(sw => sw.classList.toggle('active', sw.dataset.color === conn.color));
    // 线宽
    const lwSlider = panel.querySelector('#cpLineWidthSlider');
    const lwVal = panel.querySelector('#cpLineWidthVal');
    if (lwSlider) lwSlider.value = conn.lineWidth;
    if (lwVal) lwVal.textContent = conn.lineWidth + 'px';
    // 箭头大小
    const asSlider = panel.querySelector('#cpArrowSizeSlider');
    const asVal = panel.querySelector('#cpArrowSizeVal');
    if (asSlider) asSlider.value = conn.arrowSize || 8;
    if (asVal) asVal.textContent = (conn.arrowSize || 8) + 'px';
    // 线型样式
    panel.querySelectorAll('.cp-style-btn').forEach(b => b.classList.toggle('active', b.dataset.style === conn.lineStyle));
    // 箭头类型
    panel.querySelectorAll('.cp-arrow-row').forEach(row => {
      const isStart = row.querySelector('.cp-prop-label').textContent.trim() === '起点';
      const val = isStart ? conn.startArrow : conn.endArrow;
      row.querySelectorAll('.cp-arrow-btn').forEach(b => b.classList.toggle('active', b.dataset.arrow === val));
    });
  },
  ...
  _positionConnPanel() {
    ...
    const panel = document.getElementById('connPropertyPanel');
    if (!panel) return;
    const fromRect = this._getElRect(this._selectedConnUnitIndex, conn.from);
    const toRect = this._getElRect(this._selectedConnUnitIndex, conn.to);
    if (!fromRect || !toRect) return;
    const mx = (fromRect.x + fromRect.w / 2 + toRect.x + toRect.w / 2) / 2;
    const my = (fromRect.y + fromRect.h / 2 + toRect.y + toRect.h / 2) / 2;
    const pw = panel.offsetWidth || 270;
    const ph = panel.offsetHeight || 360;
    const wr = document.getElementById('boardViewport')?.getBoundingClientRect() || { left: 0, top: 0, width: window.innerWidth, height: window.innerHeight };
    let px = (mx < wr.width / 2) ? mx + 40 : mx - pw - 40;
    let py = (my < wr.height / 2) ? my + 30 : my - ph - 30;
    px = Math.max(10, Math.min(wr.width - pw - 10, px));
    py = Math.max(10, Math.min(wr.height - ph - 10, py));
    panel.style.left = (wr.left + px) + 'px';
    panel.style.top = (wr.top + py) + 'px';
  },
  ...
  _onConnPanelDragStart(e) {
    if (e.button !== 0) return;
    this._connPanelDragging = true;
    const panel = document.getElementById('connPropertyPanel');
    if (panel) {
      const rect = panel.getBoundingClientRect();
      this._connPanelDragOffset = { x: e.clientX - rect.left, y: e.clientY - rect.top };
    }
    e.preventDefault();
  },
  ...
  // 属性面板操作
  _cpSetLineType(type) {
    const conn = this._getSelectedConn(); if (!conn) return;
    this._pushUndo({ type: 'editConnection', unitIndex: this._selectedConnUnitIndex, connId: conn.id, oldProps: JSON.parse(JSON.stringify(conn)) });
    conn.lineType = type; conn.waypoints = [];
    this.renderConnections(this._selectedConnUnitIndex);
    this._renderConnPanel();
    this.saveBoard(); StorageManager.scheduleSave();
  },
  _cpSetColor(color) {
  ...
  _cpSetLineWidth(val) {
    const conn = this._getSelectedConn(); if (!conn) return;
    this._pushUndo({ type: 'editConnection', unitIndex: this._selectedConnUnitIndex, connId: conn.id, oldProps: JSON.parse(JSON.stringify(conn)) });
    conn.lineWidth = parseInt(val);
    const valEl = document.getElementById('cpLineWidthVal'); if (valEl) valEl.textContent = val + 'px';
    this.renderConnections(this._selectedConnUnitIndex);
    this.saveBoard(); StorageManager.scheduleSave();
  },
  _cpSetArrowSize(val) {
    const conn = this._getSelectedConn(); if (!conn) return;
    this._pushUndo({ type: 'editConnection', unitIndex: this._selectedConnUnitIndex, connId: conn.id, oldProps: JSON.parse(JSON.stringify(conn)) });
    conn.arrowSize = parseInt(val);
    const valEl = document.getElementById('cpArrowSizeVal'); if (valEl) valEl.textContent = val + 'px';
    this.renderConnections(this._selectedConnUnitIndex);
    this.saveBoard(); StorageManager.scheduleSave();
  },
  _cpSetLineStyle(style) {
    const conn = this._getSelectedConn(); if (!conn) return;
    this._pushUndo({ type: 'editConnection', unitIndex: this._selectedConnUnitIndex, connId: conn.id, oldProps: JSON.parse(JSON.stringify(conn)) });
  ...
  _cpSetArrow(pos, type) {
    const conn = this._getSelectedConn(); if (!conn) return;
    this._pushUndo({ type: 'editConnection', unitIndex: this._selectedConnUnitIndex, connId: conn.id, oldProps: JSON.parse(JSON.stringify(conn)) });
    if (pos === 'start') conn.startArrow = type; else conn.endArrow = type;
    this.renderConnections(this._selectedConnUnitIndex);
    this._renderConnPanel();
    this.saveBoard(); StorageManager.scheduleSave();
  },
  _cpDeleteConn() {
    ...
    const unit = units[this._selectedConnUnitIndex];
    if (!unit) return;
    const idx = unit.connections.findIndex(c => c.id === conn.id);
    if (idx >= 0) {
      this._pushUndo({ type: 'deleteConnection', from: conn.from, to: conn.to, unitIndex: this._selectedConnUnitIndex, connectionData: JSON.parse(JSON.stringify(conn)) });
      unit.connections.splice(idx, 1);
    }
    this._selectedConnId = null;
    this._selectedConnUnitIndex = -1;
    this._hideConnPanel();
    this.renderConnections(this._selectedConnUnitIndex >= 0 ? this._selectedConnUnitIndex : 0);
    this.saveBoard();
    StorageManager.scheduleSave();
  },
  ...
  /* 画板编辑弹窗：添加遭遇范围行 */
  _addEditEncRangeRow() {
    const list = document.getElementById('editEncRangesList');
    if (!list) return;
    const rows = list.querySelectorAll('.enc-range-row');
    const lastRow = rows[rows.length - 1];
    let newMin = 1, newMax = 5;
    if (lastRow) {
      const maxEl = lastRow.querySelector('.enc-range-max');
      if (maxEl) {
        const lastMax = parseInt(maxEl.value) || 5;
        newMin = lastMax + 1;
        newMax = newMin + 4;
      }
    }
    const div = document.createElement('div');
    div.className = 'enc-range-row';
    div.style.cssText = 'display:flex;gap:6px;margin-bottom:6px;align-items:flex-start;';
    div.innerHTML = `
      <div style="display:flex;gap:4px;align-items:center;flex-shrink:0;padding-top:4px;">
        <input type="text" class="enc-range-min" value="${newMin}" style="width:45px;text-align:center;border:1px solid var(--border);border-radius:4px;padding:4px;font-size:12px;box-sizing:border-box;">
        <span style="color:var(--text-muted);">~</span>
        <input type="text" class="enc-range-max" value="${newMax}" style="width:45px;text-align:center;border:1px solid var(--border);border-radius:4px;padding:4px;font-size:12px;box-sizing:border-box;">
      </div>
      <textarea class="enc-range-content" style="flex:1;border:1px solid var(--border);border-radius:4px;padding:4px;font-size:12px;box-sizing:border-box;min-height:46px;resize:vertical;"></textarea>
      <button onclick="this.parentElement.remove()" style="border:none;background:none;color:var(--text-muted);cursor:pointer;padding:4px;flex-shrink:0;">✕</button>
    `;
    list.appendChild(div);
  },
  ...
  _boUpdateConfirmBtn() {
    ...
    // 教学：tooltip 文本变化会横向改变尺寸，气泡需跟着重新指向确认按钮
    if (typeof TutorialManager !== 'undefined' && TutorialManager.repositionCurrent) {
      TutorialManager.repositionCurrent();
    }
  },
  ...
  _boDeleteCanvasNote(noteId) {
    ...
    if (idx < 0) return;
    const note = battle.canvas.notes[idx];
    const label = note.type === 'text' ? '文字元素' : '纯文本框';
    App.showConfirm('确定要删除这个' + label + '吗？', '', '删除', () => {
    ...
    if (battle.canvas.notes) {
      battle.canvas.notes.forEach(note => {
        if (note.type === 'plaintext') {
          ...
          // 删除按钮（hover时显示）
          el.insertAdjacentHTML('beforeend', `<div class="pt-delete-btn" onclick="event.stopPropagation(); BoardManager._boDeleteCanvasNote('${note.id}')" title="删除"><svg viewBox="0 0 24 24" width="14" height="14" style="stroke:currentColor;fill:none;stroke-width:2;stroke-linecap:round;stroke-linejoin:round;"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a2 2 0 012-2h4a2 2 0 012 2v2"/></svg></div>`);
          el.addEventListener('dblclick', (e) => {
            if (e.target.closest('.pt-delete-btn')) return;
            this._boEditPlainText(note.id);
          });
          el.addEventListener('mousedown', (e) => {
            if (e.target.closest('.pt-delete-btn')) return;
            if (this._boWaitForConnectSource) {
          ...
          el.addEventListener('dblclick', (e) => {
            if (e.target.closest('.pt-delete-btn')) return;
            this._boEditTextElement(note.id);
          });
          el.addEventListener('mousedown', (e) => {
            if (e.target.closest('.pt-delete-btn')) return;
            if (this._boWaitForConnectSource) {
...
const WorldClock = {
  ...
  init() {
    ...
    if (data.collapsed) {
      document.getElementById('worldClock').classList.add('collapsed');
      const handle = document.getElementById('wcCollapseHandle');
      if (handle) handle.title = '展开时间窗';
      this._setHandleIcon(true);
    }
  ...
  _updateVisibility() {
    ...
    if (handle) handle.style.display = (AppState.currentPage === 'board') ? '' : 'none';
  },

  /* 计算世界时钟的top位置，使其紧贴流程单元标题栏底边（把手同步跟随） */
  _updatePosition() {
    const el = document.getElementById('worldClock');
    if (!el) return;
    let top = 135;
    ...
    el.style.top = top + 'px';
    const handle = document.getElementById('wcCollapseHandle');
    if (handle) handle.style.top = (top + 20) + 'px';
  },

  /* 获取/确保 worldTime 数据 */
  _getData() {
    const board = AppState.currentModule && AppState.currentModule.board;
    if (!board) return { time: 480, day: 1, logs: [], expanded: false, collapsed: false };
    if (!board.worldTime) {
      board.worldTime = { time: 480, day: 1, logs: [], expanded: false, collapsed: false };
    }
  ...
  toggleCollapsed() {
    ...
    if (willCollapse && this._expanded) {
      this._expanded = false;
      el.classList.remove('expanded');
      this._getData().expanded = false;
    }

    this._getData().collapsed = willCollapse;
    const handle = document.getElementById('wcCollapseHandle');
    if (handle) handle.title = willCollapse ? '展开时间窗' : '收起时间窗';
    this._setHandleIcon(willCollapse);
    this._setHandleAutoHidden(false);
    this._scheduleSave();
  },

  /* 切换把手图标（收起=时钟，展开=向右箭头），带丝滑过渡 */
  _setHandleIcon(collapsed) {
    const useEl = document.getElementById('wcCollapseHandleIcon');
    if (!useEl) return;
    useEl.setAttribute('href', collapsed ? '#i-clock' : '#i-chevron-r');
    const svg = useEl.closest('svg');
    if (svg) {
      svg.classList.remove('wc-handle-swap');
      void svg.offsetWidth;
      svg.classList.add('wc-handle-swap');
    }
  },

  /* 鼠标移动：接近把手圆形范围则滑出，离开则滑入隐藏 */
  _onMouseMove(e) {
    if (typeof AppState === 'undefined' || AppState.currentPage !== 'board') return;
    ...
    requestAnimationFrame(() => {
      ...
      this._updateHandleAutoHidden();
    });
  },

  /* 根据鼠标到把手中心的距离切换自动隐藏 */
  _updateHandleAutoHidden() {
    const el = document.getElementById('worldClock');
    const handle = document.getElementById('wcCollapseHandle');
    if (!el || !handle) return;
    // 时间窗收起时不参与自动隐藏（把手由兄弟选择器定位在右边缘）
    if (el.classList.contains('collapsed')) {
      this._setHandleAutoHidden(false);
      return;
    }
    const rect = el.getBoundingClientRect();
    const cx = rect.left - 16;   // 把手滑出后的中心 X（紧贴时间窗左缘）
    const cy = rect.top + 40;    // 把手中心 Y（时间窗顶部 +20 + 高度一半20）
    const dist = Math.hypot(this._lastMouseX - cx, this._lastMouseY - cy);
    this._setHandleAutoHidden(dist >= 100);
  },

  /* 设置把手自动隐藏状态（仅在状态变化时切换类） */
  _setHandleAutoHidden(hidden) {
    if (this._autoHidden === hidden) return;
    this._autoHidden = hidden;
    const handle = document.getElementById('wcCollapseHandle');
    if (handle) handle.classList.toggle('auto-hidden', hidden);
  },
...
const TutorialManager = {
  ...
  _positionHighlight(targetSelector, interactiveSelector, holeExpand = 0) {
    ...
    // 编辑器已改为 TinyMCE，原始 #editorContent 被隐藏，改用可见的包裹层定位
    const locateSelector = targetSelector === '#editorContent' ? '#editorContentWrap' : targetSelector;
    const target = document.querySelector(locateSelector);
    if (!target) {
    ...
    if (mask) {
      ...
      let clipPath = `path(evenodd, "M0,0 H${w} V${h} H0 Z`;
      merged.forEach((hole) => {
        clipPath += ` M${hole.x1},${hole.y1} H${hole.x2} V${hole.y2} H${hole.x1} Z`;
      });
      clipPath += `")`;
  ...
  /* 重新定位当前步骤的高亮框与气泡（用于目标元素尺寸/位置动态变化后） */
  repositionCurrent() {
  ...
  _cleanupListeners() {
    ...
    const allEvents = ['createStoryClicked', 'docFileSelected', 'storyCreated', 'boardEntered',
      'dbPanelOpened', 'noteAddedToDatabase', 'notesPlaced',
...
document.addEventListener('DOMContentLoaded', () => {
  /* 分屏模式下标记 body，用于 CSS 覆盖 navbar 补偿 margin */
  if (typeof SharedBridge !== 'undefined' && SharedBridge.isInIframe()) {
  ...
  ChromeManager.init();
  App.init();
  if (typeof HomeBackground !== 'undefined') HomeBackground.init();
  PerfMonitor.init();
